...
from . import CameraModel, DataField, DatasetMetadata, DepthType  # noqa: F401
...
class BaseDataset(ABC):
    ...
    def get_dataset_metadata(self) -> DatasetMetadata | None:
        """Read structured metadata of the dataset."""
        pass
    ...
    def read(
        ...
        if data_fields is None:
            ...
            for field in data_fields:
                if field not in self.available_data_fields():
                    raise ValueError(f"Unknown data field {field}. Available fields: {self.available_data_fields()}")
