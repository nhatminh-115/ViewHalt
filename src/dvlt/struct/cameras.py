...
"""Base Camera Model adapted from Nerfstudio."""
...
from dvlt.common.geometry import normalize_with_norm, radial_and_tangential_undistort
from dvlt.common.pose import multiply_poses
from dvlt.common.projection import fisheye624_unproject
...
class Cameras(TensorDataclass):
    """Cameras object holding camera calibration and camera-to-world matrices.

    The camera coordinate frame follows OpenCV convention:
        - x-axis points right
        - y-axis points down
    ...
    def __init__(
        self,
        camera_to_worlds: Tensor,
        fx: Union[Tensor, float],
        fy: Union[Tensor, float],
        cx: Union[Tensor, float],
        cy: Union[Tensor, float],
        width: Optional[Union[Tensor, int]] = None,
        height: Optional[Union[Tensor, int]] = None,
        ...
        Note on Input Tensor Dimensions: All of these tensors have items of dimensions Tensor(*num_cameras, 3, 4)
        (in the case of the c2w matrices), Tensor(*num_cameras, 6) (in the case of distortion params), or
        Tensor(*num_cameras, 1) (in the case of the rest of the elements). The dimensions before that are
    ...
    def _init_get_camera_type(
        self,
        camera_type: Union[Tensor, int, List[CameraType], CameraType],
    ) -> Tensor:
        """
    ...
    def get_image_coords(self, pixel_offset: float = 0.5, index: Optional[Tuple] = None) -> Tensor:
        ...
        Returns:
            Grid of image coordinates. Shape: (height, width, 2)
        """
        if index is None:
    ...
    def generate_rays(
        self,
        camera_indices: Union[Tensor, int, None] = None,
        coords: Optional[Tensor] = None,
        camera_opt_to_camera: Optional[Tensor] = None,
        distortion_params_delta: Optional[Tensor] = None,
        ...
        """
        # Convenience: allow camera_indices == None  -> generate rays for *all* cameras
        if camera_indices is None:
            if self.shape == ():
                camera_indices = 0  # degenerate single-camera case
            else:
                # Construct a dense grid of indices covering every camera in the batch
                grids = torch.meshgrid(*[torch.arange(s, device=self.device) for s in self.shape], indexing="ij")
                camera_indices = torch.stack(grids, dim=-1)  # (*batch_dims, ndim)

        assert isinstance(camera_indices, (torch.Tensor, int)), "camera_indices must be a tensor, int or None"
        ...
        if cameras.is_jagged and coords is None and (keep_shape is None or keep_shape is False):
            ...
            camera_indices = torch.cat(
                [
                    index.unsqueeze(0).repeat(coords.shape[0], 1)
                    for index, coords in zip(camera_indices, _coords, strict=False)
                ],
            )
    ...
    def _generate_rays_from_coords(
        ...
        Args:
            camera_indices: Camera indices of the flattened cameras object to generate rays for.
                Shape: (*num_rays, num_cameras_batch_dims)
        ...
        if not disable_distortion:
            ...
            if distortion_params is not None:
                ...
                if mask.any() and (distortion_params != 0).any():
                    coord_stack[coord_mask, :] = radial_and_tangential_undistort(
        ...
        for cam_type in cam_types:
            if CameraType.PERSPECTIVE.value == cam_type:
                ...
                # Generate rays directly in OpenCV format: Z points forward (+1.0)
                ...
                # Generate rays directly in OpenCV format: Z points forward (+cos(theta))
                ...
                # Generate rays directly in OpenCV format (x right, y down, z forward)
                ...
                directions_stack[..., 2][mask] = torch.masked_select(
                    torch.cos(theta) * torch.sin(phi), mask
                ).float()  # Flip Z for OpenCV (forward)
                ...
                directions_stack[coord_mask] = fisheye624_unproject(masked_coords, camera_params)
        ...
        if camera_opt_to_camera is not None:
            c2w = multiply_poses(c2w, camera_opt_to_camera)
        ...
        directions_stack, directions_norm = normalize_with_norm(directions_stack, -1)
        assert directions_stack.shape == (3,) + num_rays_shape + (3,)
    ...
    def rescale_output_resolution(
        self,
        scaling_factor: Union[Tensor, float, int],
        ...
        Args:
            scaling_factor: Scaling factor to apply to the output resolution.
                Shape: (*num_cameras) or (*num_cameras, 1) or float or int
    ...
    def clone(self):
        if self.metadata is None:
            ...
            for key in self.metadata:
                ...
                self.metadata[key] = self.metadata[key].clone()
    ...
    def to_dict(self) -> Dict[str, Tensor]:
        ...
        data = {
            ...
            "distortion_params": (
                self.distortion_params.cpu().numpy().tolist() if self.distortion_params is not None else None
            ),
    ...
    def get_intrinsics_matrices(self) -> Tensor:
        """Returns the intrinsic matrices for each camera.

        Returns:
            Pinhole camera intrinsics matrices. Shape: (*num_cameras, 3, 3)
        """
...
if __name__ == "__main__":
    # ---------------------------------------------------------------------
    # Ray visualizer for every camera model
    # ---------------------------------------------------------------------
    # This helper spawns a Rerun viewer and logs a grid of rays for one
    # example camera of every CameraType. Rays are color-coded by their
    # pixel index so that left/right/top/bottom-most pixels are easy to
    # spot visually.
    ...
    W, H = 16, 12  # image resolution used for the demo
    SAMPLE_STEP = 1  # >1 to subsample rays for a sparser view

    def _make_camera(cam_type: CameraType) -> Cameras:
        fx = fy = float(max(W, H))
        cx = W / 2
        cy = H / 2
        c2w = torch.eye(4, dtype=torch.float32)[:3].unsqueeze(0)  # (1, 3, 4)
        return Cameras(
            ...
            width=W,
            height=H,
            distortion_params=torch.zeros((1, 6 if cam_type != CameraType.FISHEYE624 else 12)),
            camera_type=cam_type,
        )

    rr.init(application_id="camera_ray_visualizer", spawn=True)
    rr.log("world", rr.ViewCoordinates.RDF, static=True)

    cmap = plt.get_cmap("turbo")

    for cam_type in CameraType:
        cam = _make_camera(cam_type)
        rb = cam.generate_rays()

        origins = rb.origins[::SAMPLE_STEP, ::SAMPLE_STEP].reshape(-1, 3).cpu().numpy()
        dirs = rb.directions[::SAMPLE_STEP, ::SAMPLE_STEP].reshape(-1, 3).cpu().numpy()

        n_rays = origins.shape[0]
        # color map:
        # top-left (row 0, col 0, index 0) → deep/dark blue
        # top-right (row 0, col W-1)    → still blue-ish (shifts a bit toward cyan)
        # bottom-left (row H-1, 0)    → yellow-orange / reddish-orange
        # bottom-right (row H-1,W-1, last index) → bright red
        colors = cmap(np.linspace(0.0, 1.0, n_rays))[:, :3]

        segs = np.concatenate([origins, origins + dirs], axis=-1).reshape(-1, 2, 3)

        rr.log(f"{cam_type.name.lower()}_rays", rr.LineStrips3D(strips=segs, colors=colors))
