"""ViewHalt V0.8.1: Full 14-Scan Real Compute Efficiency Benchmark.

Benchmarks all 14 physical DTU scans x 2 subsets = 28 sequences.
Policies tested:
- Fixed_K16 (steps: 16)
- Fixed_i15 (steps: 15)
- Fixed_i14 (steps: 14)
- Fixed_i13 (steps: 13)
- Fixed_i12 (steps: 12)
- Sparse_Oracle_2pct (Tol 2% active-Q / frozen-KV execution)
- Sparse_Oracle_5pct (Tol 5% active-Q / frozen-KV execution)

Each policy runs 3 warmup repetitions + 10 timed repetitions via torch.cuda.Event.
Exact token count at runtime (378x504): P = 977 tokens per view (972 patches + 4 reg + 1 cam).
"""
...
def compute_sequence_analytical_flops(k_stars, total_K=16, S=6, P=977, C=768):
    """Computes total recurrent FLOPs for a sequence under sparse execution at 378x504."""
...
def benchmark_v08_1(data_root="datasets/test/dtu", output_dir="outputs"):
    ...
    print("Loading pretrained DVLT model for full 14-scan latency benchmarking...")
    ...
    print("=" * 80)
    print("ViewHalt V0.8.1: Full 14-Scan Real Compute Efficiency Benchmark")
    print(f"Scans ({len(scans)}): {scans}")
    print(f"Total sequences: {len(scans) * len(subsets_def)} (28 sequences)")
    print(f"Policies: Fixed i in {{12, 13, 14, 15, 16}}, Sparse Oracle 2%, Sparse Oracle 5%")
    ...
    for scan_name in scans:
        for sub in subsets_def:
            ...
            print(f"\n[{seq_counter}/{total_seqs}] Benchmarking sequence: {seq_id}")
            ...
            with torch.no_grad(), accelerator.autocast():
                ...
                P = x_init.shape[1]  # 977 tokens at 378x504
                dim = x_init.shape[2]
                ...
                policies = [
                    ...
                    {"name": "Fixed_i13", "type": "fixed", "steps": 13},
    ...
    csv_path = os.path.join(output_dir, "v08_1_timing_benchmark_raw.csv")
    ...
    # Compute aggregate summary across all 28 sequences
    policies_set = ["Fixed_K16", "Fixed_i15", "Fixed_i14", "Fixed_i13", "Fixed_i12", "Sparse_Oracle_2pct", "Sparse_Oracle_5pct"]
    summary = {}
    ...
    for pol in policies_set:
        ...
        summary[pol] = {
            ...
            "mean_latency_savings_vs_k16_pct": round(float(np.mean(savings_vs_k16)), 2),
            "mean_speedup_vs_k16": round(float(np.mean(speedup_vs_k16)), 3),
            "mean_peak_vram_mb": round(float(np.mean(vram_list)), 1),
        }

    json_path = os.path.join(output_dir, "v08_1_timing_benchmark_summary.json")
