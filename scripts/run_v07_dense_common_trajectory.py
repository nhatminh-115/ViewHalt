"""ViewHalt V0.7: Dense Common-Trajectory Pareto Resolution & Freeze Oracle.

Key Objectives:
1. Decode every recurrent state in i ∈ {8, 9, 10, 11, 12, 13, 14, 15, 16} from a single K=16 common trajectory.
2. Recompute dense oracle K* over the full integer set {8..16} across tolerances (1%, 2%, 5%, 10%).
3. Execute simulation-only freeze oracle with dense K* values (tolerances 2% and 5%).
4. Save complete raw results to outputs/v07_raw_results.json, CSV, and freeze oracle CSV.
"""
...
def run_v07_dense_common_trajectory(
    ...
    # Dense recurrent steps: every step from 8 to 16
    eval_steps = [8, 9, 10, 11, 12, 13, 14, 15, 16]
    tolerances = [0.01, 0.02, 0.05, 0.10]

    print("=" * 80)
    print("ViewHalt V0.7: Dense Common-Trajectory Pareto Resolution")
    print(f"Scans ({len(scans)}): {scans}")
    print(f"Common Trajectory Total Steps: K=16")
    print(f"Dense Decoded Checkpoints ({len(eval_steps)}): {eval_steps}")
    print("=" * 80)
    ...
    for scan_name in scans:
        for sub in subsets_def:
            ...
            with torch.no_grad(), accelerator.autocast():
                ...
                # -------------------------------------------------------------
                # 4. Dense Oracle K* for Each View across tolerances
                # -------------------------------------------------------------
                ...
                for tol in tolerances:
                    ...
                    for v in range(S):
                        ...
                        k_star = 16
                        # Search over full dense integer set {8..16}
                        for s in eval_steps:
                ...
                # -------------------------------------------------------------
                # 5. Record Dense Common-Trajectory Records
                # -------------------------------------------------------------
                for step in eval_steps:
                    for v in range(S):
                        ...
                        # Depth delta vs previous step in eval_steps
                ...
                # -------------------------------------------------------------
                # 6. Simulation Freeze Oracle with Dense K* Values
                # -------------------------------------------------------------
                ...
                print(f"  Dense Steps AbsRel: {dict(zip(eval_steps, [round(m, 5) for m in mean_absrel_steps]))}")
                print(f"  Dense Oracle K* (5% tol): {oracle_k_by_tol[0.05]} | (2% tol): {oracle_k_by_tol[0.02]}")
    ...
    print("\n" + "=" * 80)
    print("V0.7 Dense Data Generation Complete.")
    print(f"Total Trajectory Records (Dense Steps): {len(raw_trajectory_records)}")
    ...
    # Save to JSON
    json_path = os.path.join(output_dir, "v07_raw_results.json")
    ...
    # Save CSV
    csv_path = os.path.join(output_dir, "v07_raw_results.csv")
    ...
    frz_csv_path = os.path.join(output_dir, "v07_freeze_oracle_results.csv")
...
if __name__ == "__main__":
    run_v07_dense_common_trajectory()
